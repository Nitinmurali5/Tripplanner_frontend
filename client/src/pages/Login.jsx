...
import { Mail, Lock, Plane, Loader2, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-130px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-st-primary">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-st-charcoal/5 p-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-black uppercase tracking-tighter text-st-charcoal leading-none">Elite Access</h2>
          <p className="mt-4 text-st-charcoal/50 text-[10px] uppercase font-black tracking-widest">Sign in to your global portoflio</p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin text-st-primary group-hover:text-st-charcoal" /> : 'Enter Portal'}
          </button>
        </form>

        <div className="mt-10 text-center text-[10px] uppercase font-black tracking-widest text-st-charcoal/40">
          First time here?{' '}
          <Link to="/register" className="text-st-accent hover:border-b-2 border-st-accent pb-1 transition-all">
            Join the elite
          </Link>
        </div>
      </div>
